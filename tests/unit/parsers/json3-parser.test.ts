...
describe('JSON3 Parser', () => {
  ...
  describe('ASR (Auto-generated) subtitle handling', () => {
    ...
    it('should still merge for non-ASR subtitles', () => {
      const json = JSON.stringify({
        events: [
          {
            tStartMs: 1000,
      ...
      // Non-ASR should merge duplicate cues
      expect(result.cues).toHaveLength(1);
      expect(result.cues[0].endTime).toBe(2000);
    });

    it('should adjust overlapping times for non-ASR subtitles', () => {
      const json = JSON.stringify({
        events: [
          {
            tStartMs: 1000,
      ...
      // Non-ASR mode adjusts start time to avoid overlap
      expect(result.cues[1].startTime).toBe(3000);  // Adjusted to after first cue ends
    });
  });
});
