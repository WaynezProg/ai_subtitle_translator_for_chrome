...
async function handleTranslateClick(): Promise<void> {
  ...
  try {
    ...
    // Use 'first' timing strategy (default) for ASR so translations appear
    // at the same moment the speaker begins the phrase
...
async function startRealtimeTranslation(targetLanguage: string): Promise<void> {
  ...
  try {
    ...
    //
    // TIMING STRATEGY: Use 'first' (default) for ASR subtitles to show translations immediately
    // when the speaker begins the phrase. This allows viewers to:
    // - Read the full translation while listening to the original audio
    // - See translations at the same moment as the original speech starts
    // - Avoid perceived lag between hearing audio and seeing translation
    const originalCueCount = cues.length;
    cues = smartConsolidateASRCues(cues, isAutoGenerated ?? false);
    if (cues.length !== originalCueCount) {
      log.debug(`[Content] Consolidated ASR cues: ${originalCueCount} → ${cues.length} cues (using 'first' timing strategy)`);
    }
  ...
  if (cues.length === 0 && currentAdapter) {
    try {
      ...
      if (tracks.length > 0) {
        ...
        // Consolidate ASR cues for better alignment
        // Use 'first' timing strategy (default) for ASR so translations appear
        // when the speaker begins the phrase
