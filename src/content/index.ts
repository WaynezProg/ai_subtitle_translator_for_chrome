...
import { mapUploadedCuesToOriginal } from '../shared/utils/subtitle-upload';
import type { SRTGenerationMode } from '../shared/utils/srt-generator';
import type { SubtitleState } from './ui/settings-panel';
...
async function handleSubtitleUpload(content: string, filename: string): Promise<void> {
  ...
  try {
    ...
    // Get current original cues
    const originalCues: Cue[] = preTranslatedCuesWithTiming.length > 0
      ? preTranslatedCuesWithTiming.map((tc, idx) => ({
          index: idx,
          startTime: tc.startTime,
          endTime: tc.endTime,
    ...
    if (originalCues.length === 0) {
      // No existing cues - use uploaded cues as new translations
      showInfoToast('沒有原始字幕，將直接使用上傳的字幕');
      ...
      // Update realtime translator if active
    ...
    const mappingResult = mapUploadedCuesToOriginal(originalCues, parseResult.cues);
    
    if (mappingResult.warning) {
      showInfoToast(mappingResult.warning);
    }
    
    console.log(`[Content] Mapped ${mappingResult.mappedCount}/${mappingResult.originalCount} cues`);
    
    // Update the translation data
    if (preTranslatedCuesWithTiming.length > 0) {
      // Update realtime mode cues
      for (let i = 0; i < mappingResult.mappedCues.length && i < preTranslatedCuesWithTiming.length; i++) {
        const mapped = mappingResult.mappedCues[i];
        if (mapped.translatedText) {
          preTranslatedCuesWithTiming[i].translatedText = mapped.translatedText;
          // Also update the lookup map
          preTranslatedCuesMap.set(preTranslatedCuesWithTiming[i].originalText, mapped.translatedText);
        }
      }
      
      // Update realtime translator
      if (realtimeTranslator) {
        realtimeTranslator.updateTranslatedCues(preTranslatedCuesWithTiming);
        realtimeTranslator.refreshCurrentCue();
      }
    } else {
      // Update batch mode cues
      translatedCues = mappingResult.mappedCues;
    ...
    // Update settings panel subtitle state
    settingsPanel?.updateSubtitleState(getSubtitleState());
    
    showSuccessToast(`已載入翻譯: ${mappingResult.mappedCount} 句`);
