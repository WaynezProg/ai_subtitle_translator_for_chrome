...
async function handleTranslateClick(): Promise<void> {
  ...
  // IMPORTANT: Lock must be acquired inside try block to ensure release on error
  if (translationStartLock) {
    log.debug('[Content] Batch translation start already in progress');
    return;
  }
  ...
  try {
    ...
    let cues = parseSubtitle(rawSubtitle.content, rawSubtitle.format, track.isAutoGenerated);
    if (cues.length === 0) {
      throw new Error('無法解析字幕內容');
    }

    // IMPORTANT: Sort cues by startTime before consolidation
    // The ASR consolidator requires sorted input for correct gap calculation
    cues = cues.slice().sort((a, b) => a.startTime - b.startTime);
    cues = cues.map((cue, idx) => ({ ...cue, index: idx }));

    // Consolidate ASR cues for better alignment
    // Use 'weighted' timing strategy for ASR to balance timing alignment
    // This ensures consistency with realtime translation mode
...
async function startRealtimeTranslation(targetLanguage: string): Promise<void> {
  ...
  // Acquire lock to prevent race conditions
  // IMPORTANT: Lock must be checked first, then all async operations inside try block
  // to ensure lock is released on any error (network, permission, etc.)
  if (translationStartLock) {
  ...
  try {
    ...
    // For ASR subtitles, consolidate fragmented word-by-word segments into logical sentences
    // This improves bilingual display alignment by ensuring original text and translation
    // represent the same logical sentence unit instead of individual words
    //
    // TIMING STRATEGY: Use 'weighted' for ASR subtitles to balance timing alignment.
    // - 'first' shows translation too early (before speaker finishes)
    // - 'last' shows translation too late (misses part of audio)
    // - 'weighted' balances by considering text length distribution, showing the subtitle
    //   closer to when the bulk of the sentence has been spoken
    const originalCueCount = cues.length;
    cues = smartConsolidateASRCues(cues, isAutoGenerated ?? false, {
      timingStrategy: 'weighted',  // Balance timing based on text length distribution
    });
    if (cues.length !== originalCueCount) {
      log.debug(`[Content] Consolidated ASR cues: ${originalCueCount} → ${cues.length} cues (using 'weighted' timing strategy)`);
    }
          ...
          if (result.status === 'fulfilled') {
            ...
            for (const translatedCue of result.value.cues) {
              ...
              if (original && translated) {
                ...
                // Fallback to text-based matching only if index matching failed
                // IMPORTANT: Only update if this cue hasn't been translated yet (avoid duplicates)
                // This matches the safety check in Phase 1 to prevent overwriting correct translations
                for (const timingCue of preTranslatedCuesWithTiming) {
                  if (timingCue.originalText.trim() === original && timingCue.translatedText === timingCue.originalText) {
                    timingCue.translatedText = translated;
                    batchSuccessCount++;
  ...
  if (cues.length === 0 && currentAdapter) {
    try {
      ...
      if (tracks.length > 0) {
        const track = tracks[0];
        const rawSubtitle = await currentAdapter.fetchSubtitle(track as Parameters<typeof currentAdapter.fetchSubtitle>[0]);
        let parsedCues = parseSubtitle(rawSubtitle.content, rawSubtitle.format, track.isAutoGenerated);

        // IMPORTANT: Sort cues by startTime before consolidation
        // The ASR consolidator requires sorted input for correct gap calculation
        parsedCues = parsedCues.slice().sort((a, b) => a.startTime - b.startTime);
        parsedCues = parsedCues.map((cue, idx) => ({ ...cue, index: idx }));
 ...
 * - Shift + N: Toggle native subtitle visibility (show/hide native subtitles)
 */
function setupTimeOffsetKeyboardShortcuts(): void {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    switch (event.key) {
      ...
      case 'n':
      case 'N':
        // Toggle native subtitle visibility
        {
          const currentHideNative = currentRenderOptions.hideNativeSubtitles !== false;
          const newHideNative = !currentHideNative;
          currentRenderOptions.hideNativeSubtitles = newHideNative;
          saveRenderOptions(currentRenderOptions);  // Persist the setting
          realtimeTranslator.updateRenderOptions({ hideNativeSubtitles: newHideNative });
          settingsPanel?.updateSettings({ hideNativeSubtitles: newHideNative });
          showInfoToast(newHideNative ? '原生字幕已隱藏' : '原生字幕已顯示（雙層模式）');
        }
        handled = true;
        break;
    }
  ...
  log.debug('[Content] Time offset keyboard shortcuts enabled (Shift + Arrow keys, Shift + N)');
}
