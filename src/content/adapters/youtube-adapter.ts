...
export class YouTubeAdapter implements PlatformAdapter {
  ...
  async getSubtitleTracks(): Promise<SubtitleTrack[]> {
    ...
    // Proactively fetch content for tracks that don't have content yet
    // This is critical for non-ASR subtitles which YouTube doesn't preload
    await this.prefetchSubtitleContent(tracks);

    // Sort tracks: prioritize tracks with captured content, then by type
    // This ensures we use subtitles that are actually available
    tracks.sort((a, b) => {
      const aKey = `${a.language}-${a.isAutoGenerated ? 'asr' : 'manual'}`;
      const bKey = `${b.language}-${b.isAutoGenerated ? 'asr' : 'manual'}`;
      const aHasContent = this.capturedSubtitles.get(aKey)?.content?.length ?? 0;
      const bHasContent = this.capturedSubtitles.get(bKey)?.content?.length ?? 0;

      // First: tracks with content come first
      if (aHasContent > 0 && bHasContent === 0) return -1;
      if (bHasContent > 0 && aHasContent === 0) return 1;

      // Second: if both have or both lack content, non-ASR (manual) comes first
    ...
    // Update isDefault after sorting
    tracks.forEach((track, index) => {
      track.isDefault = index === 0;
    });

    log.info(`Found ${tracks.length} subtitle tracks`, {
      firstTrack: tracks[0] ? {
        language: tracks[0].language,
        isAutoGenerated: tracks[0].isAutoGenerated,
        hasContent: (this.capturedSubtitles.get(`${tracks[0].language}-${tracks[0].isAutoGenerated ? 'asr' : 'manual'}`)?.content?.length ?? 0) > 0,
      } : null,
    });
  ...
  /**
   * Proactively fetch subtitle content for tracks that don't have content yet.
   * This is essential for non-ASR subtitles which YouTube doesn't auto-load.
   */
  private async prefetchSubtitleContent(tracks: SubtitleTrack[]): Promise<void> {
    const fetchPromises: Promise<void>[] = [];

    log.info(`Checking ${tracks.length} tracks for prefetch`, {
      capturedKeys: Array.from(this.capturedSubtitles.keys()),
    });

    for (const track of tracks) {
      const key = `${track.language}-${track.isAutoGenerated ? 'asr' : 'manual'}`;
      const existing = this.capturedSubtitles.get(key);

      log.debug(`Track check: ${key}`, {
        hasExisting: !!existing,
        hasContent: existing?.content?.length ?? 0,
        hasUrl: !!track.url,
      ...
      if (existing?.content && existing.content.length > 0) {
        log.debug(`Skipping ${key}: already has content`);
        continue;
      }

      // Skip if no URL available
      if (!track.url) {
        log.debug(`Skipping ${key}: no URL`);
        continue;
      }

      // Fetch this track's content
      log.info(`Adding ${key} to prefetch queue (isASR: ${track.isAutoGenerated})`);
      const promise = this.fetchAndCacheSubtitle(track, key);
      fetchPromises.push(promise);
    }

    if (fetchPromises.length > 0) {
      log.info(`Prefetching ${fetchPromises.length} subtitle tracks`);
      ...
      log.info(`Prefetch complete: ${fulfilled} success, ${rejected} failed`);
    } else {
      log.info('No tracks need prefetching');
    }
  }

  /**
   * Fetch and cache a single subtitle track
   */
  private async fetchAndCacheSubtitle(track: SubtitleTrack, key: string): Promise<void> {
    try {
      ...
      log.info(`Prefetching subtitle: ${key}`, {
      ...
      log.debug(`Prefetch response for ${key}`, {
        status: response.status,
        ok: response.ok,
      });

      if (!response.ok) {
        log.warn(`Failed to prefetch ${key}: HTTP ${response.status}`);
        return;
      }

      const content = await response.text();
      if (content && content.length > 0) {
        this.capturedSubtitles.set(key, {
          url: track.url,
          language: track.language,
          label: track.label,
        ...
        log.info(`Successfully prefetched: ${key}`, {
          length: content.length,
          format: this.detectFormatFromContent(content),
        });
      } else {
        log.warn(`Prefetch returned empty content for ${key}`);
      }
    } catch (error) {
      log.error(`Prefetch error for ${key}`, { error: String(error) });
    }
  }
