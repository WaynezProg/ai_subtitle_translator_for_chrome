...
const SUBTITLE_STYLES = `
  ...
  .ai-subtitle-text {
    ...
    /* Quick transition for snappy subtitle changes */
  ...
  .ai-subtitle-translated {
    display: block;
    font-weight: 600;
    /* Ensure text doesn't cause layout shifts during progressive reveal */
    min-height: 1.5em;
  }
...
export class RealtimeTranslator {
   ...
   *
   * For consolidated ASR subtitles, we use an intelligent "grace period" approach:
   * - If currentTime is within a cue's time range, return that cue
   * - If currentTime is in a gap between cues:
   *   - For short gaps (≤500ms): extend previous cue seamlessly
   *   - For medium gaps (500-1000ms): extend up to 400ms
   *   - For longer gaps (1000-2000ms): extend up to 300ms
   *   - For very long gaps (>2000ms): show nothing (intentional pause)
   *
  ...
  // Animation state (kept for cleanup purposes)
  private revealAnimationId: number | null = null;

  /**
   * Simple overlay update - shows subtitle text immediately.
   * Professional subtitles appear instantly at the start of each cue.
   *
  ...
  private updateOverlaySimple(
    ...
    // Professional subtitle approach: Show full text immediately
    // This matches how professionally edited subtitles work:
    // - Text appears instantly when the speaker starts the sentence
    // - Text remains visible for the full cue duration
    // - No animation or gradual reveal (which looks unprofessional)
    //
    // The ASR consolidator already groups fragmented words into proper sentences,
    // so the translation appears at the natural sentence boundary.
    this.stopRevealAnimation();
    if (this.translatedSpan) {
  ...
  /**
   * Stop any pending reveal animation (cleanup method)
   */
